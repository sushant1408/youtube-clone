...
export const POST = async (req: Request) => {
  ...
  if (!muxSignature) {
    return new Response("No sugnature found.", { status: 401 });
  }
  ...
  switch (payload.type as WebhookEvent["type"]) {
    ...
    case "video.asset.ready": {
      ...
      if (!uploadedThumbnail.data) {
        return new Response("Failed to upload the thumbnail.", { status: 500 });
      } else if (!uploadedPreview.data) {
        return new Response("Failed to upload the preview.", { status: 500 });
      }
